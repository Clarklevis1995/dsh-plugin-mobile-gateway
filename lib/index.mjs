...
function isPrivateNetworkHostname(hostname) {
  ...
  // Tailscale MagicDNS names (the tailnet's own DNS suffix) resolve only for
  // tailnet members and survive tailnet address changes, so a ws:// URL on one
  // is the same class of private overlay transport as .local. The suffix must
  // be the final label, so a lookalike public domain such as
  // "host.ts.net.example.com" is still treated as public.
  ...
  if (octets.length === 4 && octets.every((part) => Number.isInteger(part) && part >= 0 && part <= 255)) {
    return octets[0] === 10 ||
      ...
      // RFC 6598 shared address space (100.64.0.0/10). It is not publicly
      // routable, and Tailscale assigns every tailnet node an address from it,
      // so a ws:// URL there is a private overlay transport like RFC 1918 LAN.
