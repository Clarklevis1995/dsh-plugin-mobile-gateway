      ...
      return React.createElement('div', { style: styles.backdrop, onMouseDown: () => setOpen(false) },
        React.createElement('aside', { style: styles.panel, onMouseDown: (event) => event.stopPropagation(), role: 'dialog', 'aria-modal': true, 'aria-label': '移动设备管理' },
          ...
          React.createElement('section', { style: styles.card },
            ...
            React.createElement('input', { style: styles.input, value: publicUrl, onChange: (event) => { automaticUrlRef.current = ''; setPublicUrl(event.target.value) }, placeholder: 'wss://192.168.1.10:3081/ws/mobile', spellCheck: false }),
            ...
            React.createElement('p', { style: { ...styles.muted, margin: '10px 0 0' } }, '同一局域网可直接使用私有地址的 wss://；公网地址仍必须使用 wss://。配对码只能使用一次，并在 5 分钟内过期。')),
